...
export default function AssetClassChart() {
    ...
    const CreateDetailChart = async () => {
        // 차트 데이터 설정
        const filteredList = rawData.filter((item) => item.asset_big_class == chartItemData);
        const data = filteredList.map(({ asset_name, amount }) => ({ asset_name, amount }))
            ...
            <Paper sx={{ width: '100%', mb: 2 }}>
                <Toolbar
                    ...
                    <Typography
                        ...
                        id="tableTitle"
                        component="div">
                        분류별 자산차트<br />(총합 : {parseNumber(sumChartData)}원)
                    </Typography>
