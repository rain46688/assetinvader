...
interface EnhancedTableToolbarProps {
  ...
  setOrderBy: React.Dispatch<React.SetStateAction<keyof InterestData>>;
  validationList: InterestValidation[];
  setValidationList: React.Dispatch<React.SetStateAction<InterestValidation[]>>;
  addStatus: boolean;
...
export function EnhancedTableToolbar(props: EnhancedTableToolbarProps) {
  ...
  const list = useAppSelector(state => state.interestReducer); // Redux 상태에서 필요한 데이터 읽어오기
  ...
  const handleDeleteList = async () => {
    ...
    selected.forEach(async (id) => {
      const result = await sendDelete('interest/delete_interest/' + id);
  ...
  const handleCompleteList = async () => {
    ...
    const data = JSON.stringify({
      ...
      "occurrence_date": new_data.occurrence_date
    });

    const result = await sendPost(data, 'interest/add_interest');
    if (result.status === 'success') {
      setSnack(true);
