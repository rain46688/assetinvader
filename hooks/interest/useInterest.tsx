...
import { InterestData, InterestValidation, AssetName, createData } from "@/redux/interest/Interest";
...
export const useInterest = () => {
    ...
    // 정렬 기준 관련
    const [orderBy, setOrderBy] = useState<keyof InterestData>('occurrence_date');
    // 데이터 선택 관련
    ...
    // 유효성 검사 리스트
    const [validationList, setValidationList] = useState<InterestValidation[]>([]);
    // 유효성 검사 성공 여부
    ...
    const rows = useAppSelector(state => state.interestReducer);
    ...
    const getList = async (id: string) => {
        console.log('=== getList === ');
        const res = await sendGet('/interest/getlist_interest_main/' + id);
        if (res.status === 'success') {
            ...
            // 유효성 검사 리스트
            const valList: InterestValidation[] = [];
            // 데이터 저장
            const list = res.data;
            // 데이터 변환

            const newList = list.map((item: InterestData, index: number) => {
                // asset 값이 없을 경우 건너뜀
            ...
            const filteredList = newList.filter((item: InterestData) => item !== undefined);

            // 유효성 검사 리스트 저장
            setValidationList(valList);
            // 데이터 저장
            dispatch(setInterestList(filteredList));
        } else {
    ...
    const handleDataChange = (event: any, id: number, index: number, field: string) => {
        ...
        // 수정된 배열을 설정
        dispatch(setInterestList(updatedRows));
    };
